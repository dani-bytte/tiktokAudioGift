        ...
        <div className="flex flex-col gap-4 py-4">
          <div className="space-y-2">
            <label
              ...
              title="Tempo inicial em cada início de overlay"
            >
            ...
            <Input
              ...
              onChange={(e) =>
                setTempTimerInitialValue(parseInt(e.target.value) || 0)
              }
            />
            <p className="text-xs text-muted-foreground">
              O timer vai recomeçar nesse tempo quando for reiniciado.
            </p>
          </div>
          <div className="space-y-2">
            <label
              className="text-sm font-medium"
              title="Quanto tempo soma para cada 1 de diamante"
            >
              Tempo por Moeda (segundos)
            </label>
            <Input
              type="number"
              ...
              onChange={(e) =>
                setTempTimerSecondsPerCoin(parseInt(e.target.value) || 0)
              }
            />
            <p className="text-xs text-muted-foreground">
              Fator padrão multiplicador de tempo pelas moedas do presente
              recebido.
            </p>
          </div>
        </div>
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="flex flex-col gap-4 py-4">
          <div className="space-y-2">
            ...
            />
            <p className="text-xs text-muted-foreground">
              Use valores negativos para reduzir o tempo (ex: -10).
            </p>
          </div>
        </div>
        <DialogFooter>
...
interface LogEntry {
  id: number;
  type: "info" | "gift" | "chat" | "member" | "like" | "error";
  message: string;
  time: string;
}
...
function App() {
  ...
  const [giftMappings, setGiftMappings] = useState<
    Record<string, GiftAudioMapping>
  >({});
  const [mediaDialogOpen, setMediaDialogOpen] = useState(false);
  const [selectedMediaGiftId, setSelectedMediaGiftId] = useState<string | null>(
    null,
  );
  ...
  const [overlayShowLeaderboard, setOverlayShowLeaderboard] = useState(true);
  ...
  const [isTimerSettingsDialogOpen, setIsTimerSettingsDialogOpen] =
    useState(false);
  ...
  const addLog = useCallback((type: LogEntry["type"], message: string) => {
    ...
    setLogs((prev) => [
      ...prev.slice(-99),
      { id: logIdCounter++, type, message, time },
    ]);
  }, []);
  ...
  const loadSettings = useCallback(async () => {
    try {
      ...
      setOverlayShowLeaderboard(settings.overlayShowLeaderboard ?? true);
  ...
  useEffect(() => {
    if (!window.electronAPI) {
      ...
      addLog(
        "info",
        "Running in development mode (Electron API not available)",
      );
      return;
    }
    ...
    const handleRoomStats = (stats: { viewerCount: number }) => {
      setRoomInfo((prev) =>
        prev ? { ...prev, viewerCount: stats.viewerCount } : null,
      );
    };
    ...
    const handleLike = (event: { nickname: string; likeCount: number }) => {
      addLog("like", `❤️ ${event.nickname} curtiu (${event.likeCount})`);
    };
    ...
    window.electronAPI.on("tiktok:like", handleLike);
    ...
    return () => {
      ...
      window.electronAPI.off("tiktok:like", handleLike);
    };
  }, [addLog]);
  ...
  const handleConnect = async () => {
    if (connectionStatus === "connected") {
      ...
      try {
        ...
        const uniqueGifts = gifts.filter(
          (gift: any, index: number, self: any[]) =>
  ...
  const handleCopyTimerUrl = async () => {
    await navigator.clipboard.writeText(timerOverlayUrl);
    addLog("info", "Timer overlay URL copied!");
  };
    ...
    try {
      ...
      setAudioQueueProgress((prev) => {
        ...
        if (
          ...
          prev.remaining === progress.remaining &&
          prev.estimatedSeconds === estimatedSeconds
        ) {
          return prev;
        }
  ...
  const handleAudiosSaved = async (paths: string[]) => {
    if (selectedGiftId && selectedGiftName) {
      ...
      const added = paths.filter(
        (p) => !existing?.audioFiles.some((e) => e.path === p),
      ).length;
      const removed =
        (existing?.audioFiles.length || 0) -
        paths.filter((p) => existing?.audioFiles.some((e) => e.path === p))
          .length;
  ...
  const handleUpdateMapping = async (
    giftId: string,
    updates: Partial<GiftAudioMapping>,
  ) => {
  ...
  const handleSelectMedia = (giftId: string) => {
    setSelectedMediaGiftId(giftId);
    setMediaDialogOpen(true);
  };

  const handleMediaSaved = async (mediaPath: string | undefined) => {
    if (!selectedMediaGiftId) return;

    await handleUpdateMapping(selectedMediaGiftId, {
      mediaPath,
      mediaEnabled: Boolean(mediaPath),
    });

    if (mediaPath) {
      toast.success("Mídia vinculada ao presente");
    } else {
      toast.info("Mídia removida do presente");
    }
  };
  ...
  const handleOverlayShowLeaderboardChange = async (enabled: boolean) => {
    setOverlayShowLeaderboard(enabled);
    if (window.electronAPI?.setOverlayShowLeaderboard) {
      await window.electronAPI.setOverlayShowLeaderboard(enabled);
    }
  };
  ...
  const filteredMappings = useMemo(() => {
    return Object.values(giftMappings).filter((m) =>
      m.giftName.toLowerCase().includes(searchTerm.toLowerCase()),
    );
    ...
    <div className="flex flex-col h-screen bg-background">
      <header className="flex items-center justify-between px-6 py-4 border-b border-border bg-card">
        ...
        <div className="flex items-center gap-2">
          <Badge
            variant={overlayConnected > 0 ? "default" : "secondary"}
            className={overlayConnected > 0 ? "bg-green-600" : ""}
          >
            OBS:{" "}
            {overlayConnected > 0
      ...
      <main className="flex flex-1 overflow-hidden">
            ...
            <div className="p-4 space-y-4">
              <Card>
                ...
                <CardContent className="space-y-3">
                  <div className="space-y-2">
                    ...
                    <div className="flex">
                      <span className="h-9 px-3 flex items-center bg-muted text-muted-foreground rounded-l-md border border-r-0 border-input text-sm">
                        @
                      </span>
                      <Input
                        type="text"
                  ...
                  {connectionStatus === "connected" && roomInfo && (
                    <div className="flex items-center gap-3 p-3 rounded-lg bg-green-500/10 border border-green-500/20">
                      <Avatar size="lg">
                        <AvatarImage
                          src={roomInfo.profilePictureUrl}
                          alt={roomInfo.nickname}
                        />
                        <AvatarFallback>
                          {roomInfo.nickname?.charAt(0)?.toUpperCase() || "?"}
                        </AvatarFallback>
                      </Avatar>
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-xs text-muted-foreground">
                          {roomInfo.viewerCount} viewers
                        </p>
                      </div>
                      <Badge
                        variant="secondary"
                        className="bg-green-500/20 text-green-400 shrink-0"
                      >
                        LIVE
                      </Badge>
                    </div>
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button
                        variant="secondary"
                        size="icon"
                        onClick={handleCopyTimerUrl}
                        title="Copy Timer URL"
                      >
                        📋
                      </Button>
                    </div>
                  ...
                  <div className="flex items-center justify-between rounded-md border border-border p-2">
                    <div>
                      <p className="text-sm font-medium">Mostrar ranking no overlay principal</p>
                      <p className="text-xs text-muted-foreground">Liga/desliga apenas visualmente no overlay.</p>
                    </div>
                    <Switch
                      checked={overlayShowLeaderboard}
                      onCheckedChange={handleOverlayShowLeaderboardChange}
                    />
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                  ...
                  <CardTitle className="text-base flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <Button
                        ...
                        onClick={() => setIsTimerSettingsDialogOpen(true)}
                      >
                        ⚙️
                      </Button>
                      <Switch
                      ...
                      <Badge
                        ...
                        className={
                          timerEnabled
                            ? "bg-green-600 font-mono text-[10px]"
                            : "font-mono text-[10px]"
                        }
                      >
                        {timerEnabled ? "ON" : "OFF"}
                ...
                <CardContent className="space-y-3">
                  ...
                  <p className="text-[10px] text-muted-foreground pt-2 border-t border-border mt-3 text-center">
                    Configurações de tempo movidas para a ⚙️.
                  </p>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  <div className="relative" data-dropdown="test-gift">
                    ...
                    <button
                      onClick={() =>
                        setTestGiftDropdownOpen(!testGiftDropdownOpen)
                      }
                    ...
                    >
                          ...
                          <>
                            {(() => {
                              const gift = availableGifts.find(
                                (g) => g.id.toString() === selectedTestGiftId,
                              );
                              return gift?.imageUrl ? (
                                <img
                                  src={gift.imageUrl}
                                  alt={
                                    giftMappings[selectedTestGiftId]?.giftName
                                  }
                      ...
                      <span
                        className={`transition-transform ${testGiftDropdownOpen ? "rotate-180" : ""}`}
                      >
                        ▼
                      </span>
                    </button>
                    ...
                    {testGiftDropdownOpen && (
                      <div className="absolute top-full left-0 right-0 mt-1 bg-background border border-input rounded-md shadow-lg z-50 max-h-64 overflow-y-auto">
                        {Object.entries(giftMappings).length === 0 ? (
                          ...
                          Object.entries(giftMappings).map(
                            ([giftId, mapping]) => (
                              <button
                                key={giftId}
                                onClick={() => {
        ...
        <section className="flex-1 flex flex-col p-4 overflow-hidden">
          ...
          <Tabs defaultValue="configured" className="flex-1 flex flex-col">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-1 pb-16">
                  {filteredMappings.length === 0 ? (
                    <div className="col-span-full flex flex-col items-center justify-center py-12 text-muted-foreground">
                      <p className="text-lg">
                        No audio mappings configured yet
                      </p>
                      <p className="text-sm">
                        Connect to a live stream and configure audio for gifts
                      </p>
                    </div>
                  ) : (
                    filteredMappings.map((mapping) => {
                      ...
                      return (
                        <Card
                          key={mapping.giftId}
                          className={`gap-0 py-0 overflow-hidden ${hasAudio ? "border-l-4 border-l-green-500" : "border-l-4 border-l-muted"}`}
                        >
                          <div className="flex items-center justify-between p-3 bg-muted/30">
                            <div className="flex items-center gap-2 min-w-0">
                              {(() => {
                                const gift = availableGifts.find(
                                  (g) => g.id.toString() === mapping.giftId,
                                );
                                return gift?.imageUrl ? (
                                  <img
                                    src={gift.imageUrl}
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                              <Switch
                                ...
                                onCheckedChange={(checked) =>
                                  handleUpdateMapping(mapping.giftId, {
                                    enabled: checked,
                                  })
                                }
                              />
                              <Badge
                                variant={
                                  mapping.enabled ? "default" : "secondary"
                                }
                                className={
                                  mapping.enabled ? "bg-green-600" : ""
                                }
                              >
                          ...
                          <CardContent className="p-3 space-y-3">
                            <div className="space-y-2">
                              <div className="flex items-center justify-between">
                                <span className="text-xs font-medium text-muted-foreground flex items-center gap-1">
                                  ...
                                  <Badge
                                    variant="outline"
                                    className="ml-1 h-5 text-[10px] bg-background"
                                  >
                                    {audioCount}{" "}
                                    {audioCount === 1 ? "Áudio" : "Áudios"}
                                  </Badge>
                                </span>
                                <Button
                                  variant="outline"
                            ...
                            <div className="pt-2 mt-2 border-t border-border flex items-center justify-between gap-2">
                              <span
                                ...
                                title="Se preenchido, adicionará exatamente essa quantidade de MINUTOS e ignorará o fator multiplicador global de moedas. Aceita decimais (ex: 0.5 = 30 seg)."
                              >
                                ⏱️ Timer Override
                              </span>
                              <div className="flex items-center gap-2">
                                <TimerOverrideInput
                                  ...
                                  onSave={(id, val) =>
                                    handleUpdateMapping(id, {
                                      customTimerAmount: val,
                                    })
                                  }
                                />
                            ...
                            <div className="pt-2 mt-2 border-t border-border flex items-center justify-between gap-2">
                              <span className="text-xs font-medium text-muted-foreground flex items-center gap-1">
                                🎬 Media
                                {mapping.mediaPath ? (
                                  <Badge variant="outline" className="ml-1 h-5 text-[10px] bg-violet-500/10 text-violet-600">
                                    On
                                  </Badge>
                                ) : (
                                  <Badge variant="outline" className="ml-1 h-5 text-[10px] opacity-50">
                                    Off
                                  </Badge>
                                )}
                              </span>
                              <div className="flex items-center gap-1">
                                <Button
                                  ...
                                  onClick={() => handleSelectMedia(mapping.giftId)}
                                >
                                  {mapping.mediaPath ? "🔄 Gerenciar" : "+ Mídia"}
                                </Button>
                              </div>
                            </div>
                          </CardContent>
              ...
              <ScrollArea className="h-[calc(100vh-220px)]">
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 p-1 pb-16">
                  {processedAvailableGifts.map((gift, index) => {
                    const imageUrl =
                      gift.imageUrl || gift.image?.url_list?.[0] || "";
                    return (
                      <div
                        ...
                        onClick={() =>
                          handleSelectAudio(gift.id?.toString(), gift.name)
                        }
                      >
                        {imageUrl ? (
                          <img
                  ...
                  {availableGifts.filter((g) => !giftMappings[g.id?.toString()])
                    .length === 0 && (
                    <div className="col-span-full flex flex-col items-center justify-center py-12 text-muted-foreground">
                      <p className="text-lg">No available gifts</p>
                      <p className="text-sm">
                        Connect to a TikTok live stream to load gifts
                      </p>
                    </div>
                  )}
                </div>
            ...
            <TabsContent
              value="library"
              className="flex-1 overflow-hidden p-0 m-0 border-none data-[state=active]:flex flex-col"
            >
          ...
          {timerEnabled && (
            <Card className="bg-primary/5 border-primary/20">
              <CardHeader className="pb-2 pt-3">
                <CardTitle className="text-sm flex items-center justify-between">
                  ...
                  {isTimerPaused && (
                    <Badge
                      variant="secondary"
                      className="text-[10px] bg-yellow-500/20 text-yellow-600"
                    >
                      Paused
                    </Badge>
                  )}
              ...
              <CardContent className="pb-3 space-y-3">
                <div
                  className={`text-4xl font-bold text-center font-mono tracking-wider text-shadow-sm ${isTimerPaused ? "text-yellow-500" : "text-primary"}`}
                >
                  {formatTime(timeLeft)}
                </div>
                <Button
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-base flex items-center justify-between">
                ...
                <Badge
                  variant={
                    audioQueueProgress.remaining > 0 ? "default" : "secondary"
                  }
                >
                  ...
                  <div className="text-sm space-y-2">
                    <div className="flex justify-between text-muted-foreground">
                      ...
                      <span className="font-medium">
                        {audioQueueProgress.current}/{audioQueueProgress.total}
                      </span>
                    </div>
                    {/* Progress Bar */}
                    <Progress
                      value={
                        (audioQueueProgress.current /
                          audioQueueProgress.total) *
                        100
                      }
                      className="h-2"
                    />
                    <div className="text-xs text-muted-foreground">
                      {audioQueueProgress.remaining} audio
                      {audioQueueProgress.remaining !== 1 ? "s" : ""} remaining
                      {audioQueueProgress.estimatedSeconds > 0 && (
                        <span className="ml-2">
                          • ~
                          {Math.floor(audioQueueProgress.estimatedSeconds / 60)}
                          :
                          {String(
                            audioQueueProgress.estimatedSeconds % 60,
                          ).padStart(2, "0")}
                        </span>
                      )}
                    </div>
      ...
      <AudioSelectionDialog
        ...
        currentPaths={
          selectedGiftId
            ? giftMappings[selectedGiftId]?.audioFiles.map((e) => e.path) || []
            : []
        }
      />
      ...
      <MediaSelectionDialog
        ...
        onOpenChange={(open) => {
          ...
          if (!open) setSelectedMediaGiftId(null);
        }}
        onSave={handleMediaSaved}
        currentPath={
          selectedMediaGiftId
            ? giftMappings[selectedMediaGiftId]?.mediaPath
            : undefined
        }
      />
